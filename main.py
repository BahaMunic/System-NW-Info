...
def run_command(args):
    ...
    try:
        ...
        return f"Error: {e.stderr.strip()}"  # Return error message if command fails
...
def search_running_processes():
    ...
    if process_name:
        ...
        if "No tasks are running" in result:
            text_area.insert(tk.END, f"لم يتم العثور على عملية باسم {process_name}.")
        else:
            text_area.insert(tk.END, result)
...
def terminate_process():
    ...
    if pid:
        result = run_command(["taskkill", "/PID", pid, "/F"])
        text_area.delete(1.0, tk.END)
        if "SUCCESS" in result:
            text_area.insert(tk.END, f"تم إنهاء العملية ذات الرقم {pid} بنجاح.")
        else:
            text_area.insert(tk.END, f"فشل في إنهاء العملية ذات الرقم {pid}: {result}")

def ping_ip():
    ip = simpledialog.askstring("تحقق من عنوان", "أدخل عنوان الجهاز لتجربته:")
    if ip:
        result = run_command(["ping", "-n", "1", ip])
        text_area.delete(1.0, tk.END)
        text_area.insert(tk.END, result)
...
def dns_lookup():
    ...
    if hostname:
        result = run_command(["nslookup", hostname])
        text_area.delete(1.0, tk.END)
        text_area.insert(tk.END, result)
...
def traceroute():
    ...
    if target:
        result = run_command(["tracert", "-d", target])
        text_area.delete(1.0, tk.END)
        text_area.insert(tk.END, result)
...
def check_port():
    ...
    if ip and port:
        command = [
            "powershell", "-Command",
            f"try {{ $tcp = New-Object System.Net.Sockets.TcpClient; $tcp.Connect('{ip}', {port}); $tcp.Close(); Write-Output 'True' }} catch {{ Write-Output 'False' }}"
        ]
...
# إضافة تذييل
footer = tk.Label(root, text="تم بواسطة وكالة التحول الرقمي بأمانة منطقة الباحة", bg="#f0f0f0", font=("Arial", 10))
